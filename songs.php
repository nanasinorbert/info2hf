...
if (isset($_POST['uj']) and isset($_POST['search'])) {
    $search  = $_POST['search'];
    header("Location:songs.php?search=$search");
}
?>
<!DOCTYPE html>
<html>
...
<body>
    ...
    include 'db.php';
    $link = opendb();
    $eredmeny = mysqli_query($link, "SELECT s.id AS id, s.songName AS songName, s.releaseYear AS releaseYear, s.songStyle AS songStyle, IFNULL(a.artistAlias, 'Ismeretlen') AS artistAlias FROM songs s LEFT OUTER JOIN artists a ON s.Artists_Id =a.id");
    ?>
